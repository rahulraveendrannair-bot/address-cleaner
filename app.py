...
def _load_lookups():
    ...
    ADDR_RE = re.compile(
        ...
        r'ul\b|d\b(?=\s*\d)|Str\b|Mah\b|Blok\b|Kat\b|Daire\b|Pasa\b|Calle\b|Esq\b|Avenida\b|Carrera\b|Edif\b|Bd\b|Av\b|Rue\b|'
    ...
    STATE_KW = re.compile(
        ...
        r'Muhafazah|Wilayat|Banteay|Commune|Arrondissement|'
        r'Administrative|Governorate|Zone|Area|Sector|Circuit|Cercle|'
        r'Subregion|Sub-region|Subcounty|Sub-county|Constituency|'
        r'Tehsil|Taluk|Mandal|Upazila|Thana|Zila|Woreda|Kebele|'
        r'Sub-city|Subcity|Subcounty|Sub-county|Arrondissement|'
        r'Maakond|Laan|Landkreis|Lan|Fylke|Amt)\b',
        re.I
    )
    ...
    # Pattern: detect P.O. Box / BP / CP prefix — numbers after these are BOX NUMBERS not postal codes
    PO_BOX_RE = re.compile(
        r'\b(?:P\.?\s*O\.?\s*Box|PO\s+Box|Post(?:al)?\s+Box|GPO\s+Box|'
        r'Private\s+Bag|Locked\s+Bag|'
        r'B\.?\s*P\.?|C\.?\s*P\.?|'
        r'Apartado(?:\s+Postal)?|Caixa\s+Postal|'
        r'Postbus|Postfach|Bo[iî]te\s+Postale)'
        r'\s*', re.I
    )
    # Pattern: detect Room/Unit/Floor/Suite prefix — numbers after these are unit numbers not postal codes
    ROOM_RE = re.compile(
        r'\b(?:Room|Rm|Unit|Suite|Ste|Apt|Apartment|Floor|Fl|Level|Lv|'
        r'Lot|Plot|Gate|Bay|Door|Kab|Pom|Liter|Lit|Korp|Korpus)'
        r'[\s\.#-]*', re.I
    )
    # City name normalisation — maps short/alternate names to canonical full names
    CITY_NORMALIZE = {
        # Thailand
    ...
    return {
        ...
        'ADDR_RE': ADDR_RE,
        'STATE_KW': STATE_KW,
        'TOWNSHIP_RE': TOWNSHIP_RE,
        'DISTRICT_RE': DISTRICT_RE,
        'TERRITORY_RE': TERRITORY_RE,
        'JUNK_POSTAL': JUNK_POSTAL,
        'PO_BOX_RE': PO_BOX_RE,
        'ROOM_RE': ROOM_RE,
        'CITY_NORMALIZE': CITY_NORMALIZE,
        'NOTE_RE': NOTE_RE,
        'ISO2_RE': ISO2_RE,
        'UK_POST_RE': UK_POST_RE,
        'US_STATES': US_STATES,
        'CA_PROVS': CA_PROVS,
        'US_STATE_EXPAND': US_STATE_EXPAND,
        'CA_PROV_EXPAND': CA_PROV_EXPAND,
        'COUNTRY_NORMALIZE': COUNTRY_NORMALIZE,
        'COUNTRY_TO_CODE': COUNTRY_TO_CODE,
        'ISO_TO_COUNTRY': ISO_TO_COUNTRY,
        'TERRITORY_TO_COUNTRY': TERRITORY_TO_COUNTRY,
        'KNOWN_STATES': KNOWN_STATES,
        'CITY_TO_COUNTRY': CITY_TO_COUNTRY,
    }

_lk = _load_lookups()
COUNTRY_RE = _lk['COUNTRY_RE']
ADDR_RE = _lk['ADDR_RE']
STATE_KW = _lk['STATE_KW']
TOWNSHIP_RE = _lk['TOWNSHIP_RE']
DISTRICT_RE = _lk['DISTRICT_RE']
TERRITORY_RE = _lk['TERRITORY_RE']
JUNK_POSTAL = _lk['JUNK_POSTAL']
PO_BOX_RE = _lk['PO_BOX_RE']
NOTE_RE = _lk['NOTE_RE']
ROOM_RE             = _lk['ROOM_RE']
CITY_NORMALIZE      = _lk['CITY_NORMALIZE']
ISO2_RE = _lk['ISO2_RE']
UK_POST_RE = _lk['UK_POST_RE']
US_STATES = _lk['US_STATES']
CA_PROVS = _lk['CA_PROVS']
US_STATE_EXPAND = _lk['US_STATE_EXPAND']
CA_PROV_EXPAND = _lk['CA_PROV_EXPAND']
COUNTRY_NORMALIZE = _lk['COUNTRY_NORMALIZE']
COUNTRY_TO_CODE = _lk['COUNTRY_TO_CODE']
ISO_TO_COUNTRY = _lk['ISO_TO_COUNTRY']
TERRITORY_TO_COUNTRY = _lk['TERRITORY_TO_COUNTRY']
KNOWN_STATES = _lk['KNOWN_STATES']
CITY_TO_COUNTRY = _lk['CITY_TO_COUNTRY']
...
def parse_city_field(cty):
    ...
    # No-comma path: check if the whole string is a village/sub-locality
    _SPEC_SUB_NC = re.compile(
        r'\b(County|District|Borough|Ward|Parish|Commune|'
        r'Rayon|Raion|Gun|Arrondissement|Tehsil|Taluk|Subdistrict|'
        r'K\u00fcla|Vald|Alevik|Dong|Ri|Myeon|'
        r'Mura|Machi|Village|Hamlet|Settlement|'
        r'Rural Settlement|Rural Locality)\b|'
        r'\w+-ku\b|\w+-gu\b|'
        r'\w+ k\u00fcla\b|\w+ vald\b',
        re.I)
    if ',' not in s and _SPEC_SUB_NC.search(s) and not STATE_KW.search(s):
        # Whole string is a village/district — goes to addr3, not city
        return '', '', s
    ...
    if ',' not in s:
        ...
        if m:
            ...
            if m.end() >= len(s.rstrip()):
                ...
                if _is_township(s):
                    return '', '', s
                # Specific sub-city units (District, County, Ward...) → ADDRESS3
                _SPEC_SUB = re.compile(
                    r'\b(County|District|Borough|Ward|Parish|Commune|'
                    r'Rayon|Raion|Gun|Arrondissement|Tehsil|Taluk|Subdistrict|'
                    r'K\u00fcla|Vald|Alevik|Dong|Ri|Myeon|'
                    r'Village|Hamlet|Settlement|Rural Settlement|Rural Locality)\b|'
                    r'\w+-ku\b|\w+-gu\b|'
                    r'\w+ k\u00fcla\b|\w+ vald\b',
                    re.I)
                if _SPEC_SUB.search(s):
                    return '', '', s
                ...
                # If first word looks like a standalone city/town name
                # and the rest is a regional qualifier, split: city=first, state=rest
                # e.g. 'PROKHOLADNY KABARDINO-BALKARIAN REPUBLIC' → city='PROKHOLADNY'
                first_word = s.split()[0] if s.split() else ''
                rest_words = ' '.join(s.split()[1:]) if len(s.split()) > 1 else ''
                # Only split if rest_words has content BEYOND the bare keyword
                # e.g. 'Gangwon Province' → rest='Province' alone → DON'T split (state=whole)
                # e.g. 'Prokholadny Kabardino-Balkarian Republic' → rest has extra words → split
                _rest_stripped = STATE_KW.sub('', rest_words).strip(' -,')
                _DIRECTIONAL = re.compile(
                    r'^(North|South|East|West|New|Greater|Upper|Lower|'
                    r'Central|Inner|Outer|Far|Middle|Northern|Southern|'
                    r'Eastern|Western|Old|Little|Great)$', re.I)
                if (first_word and re.match(r'^[A-ZА-Я]', first_word)
                        and not STATE_KW.search(first_word)
                        and not COUNTRY_RE.search(first_word)
                        and not _DIRECTIONAL.match(first_word)  # not a directional qualifier
                        and rest_words and STATE_KW.search(rest_words)
                        and _rest_stripped):  # rest must have words beyond the keyword
                    return first_word.title(), rest_words, ''
                return '', s, ''
    ...
    _SPEC_SUB = re.compile(
        r'\b(County|District|Borough|Ward|Parish|Commune|'
        r'Rayon|Raion|Gun|Arrondissement|Tehsil|Taluk|Subdistrict|'
        # Village/hamlet suffixes — these are sub-locality, not city-level
        r'K\u00fcla|Vald|Alevik|'  # Estonian: village, parish, small-town
        r'Dong|Ri|Myeon|'           # Korean: district, village, township
        r'Mura|Machi|Cho|'          # Japanese: village, town
        r'Village|Hamlet|Settlement|'
        r'Rural Settlement|Rural Locality)\b|'
        r'\w+-ku\b|\w+-gu\b|'       # Japanese ward (-ku) and Korean district (-gu)
        r'\w+ k\u00fcla\b|'       # Estonian: Turisalu küla
        r'\w+ vald\b',             # Estonian: Harku vald
        re.I)
    # Re-classify: parts matching _SPEC_SUB go to sub_idx even if they have STATE_KW
    spec_idx = {i for i in range(len(parts)) if _SPEC_SUB.search(parts[i])}
    # Adjust region_idx: remove parts that are level-2 districts
    region_idx -= spec_idx
    sub_idx   = set()
    other_raw = [parts[i] for i in range(len(parts))
                 if i not in region_idx and i not in country_idx
                 and i not in township_idx and i not in spec_idx]
    # spec_idx parts always go to sub_parts (ADDRESS3)
    sub_parts_from_spec = [parts[i] for i in sorted(spec_idx)]
    # Sub-district parts (ward, borough, -ku, etc.) → addr3, not city
    sub_parts  = sub_parts_from_spec + [p for p in other_raw if _SPEC_SUB.search(p)]
    other      = [p for p in other_raw if not _SPEC_SUB.search(p)]
    ...
    if not other and not sub_parts:
        ...
        if non_kw:
            ...
            return city, ', '.join(state_parts), ', '.join(sub_parts)
    ...
    if not other and sub_parts:
        # Only sub-district parts in 'other_raw' (e.g. 'Tokyo, Chuo-ku')
        # Pick city from region_idx — the known state IS also the city context
        non_kw2 = [parts[i] for i in sorted(region_idx) if not STATE_KW.search(parts[i])]
        if non_kw2:
            city = next((p for p in non_kw2 if p.lower() in CITY_TO_COUNTRY), non_kw2[0])
            state_parts2 = [parts[i] for i in sorted(region_idx) if parts[i].lower() != city.lower()]
            return city, ', '.join(state_parts2), ', '.join(sub_parts)
        else:
            # All parts are STATE_KW, sub_parts are districts
            return '', ', '.join(parts[i] for i in sorted(region_idx)), ', '.join(sub_parts)

    # Check if region_idx contains a city name (in CTC) that is also a province.
    # e.g. Istanbul, Ankara, Antalya are both KNOWN_STATES and cities in CTC.
    # When paired with a small district (other), the CTC city should be CITY
    # and the district should go to ADDRESS3.
    # City-provinces: names that are BOTH a city AND a province/state
    ...
    if (_region_city and other and len(other) == 1
            and not ADDR_RE.search(other[0])
            and other[0].lower() not in CITY_TO_COUNTRY
            and other[0].lower() not in KNOWN_STATES):
        # Pattern: 'Maltepe, Istanbul' — Istanbul is city, Maltepe is district
        remaining_regions = [parts[i] for i in sorted(region_idx)
                              if parts[i] != _region_city]
        return _region_city, ', '.join(remaining_regions), other[0]

    # City = first 'other' part (most specific location)
    city = other[0]
    ...
    addr3_parts    = [p for p in other if p != city] + township_parts + sub_parts
...
def extract_from_address1(a1, ex_city='', ex_postal='', ex_country=''):
    ...
    # Strip trailing semicolons/whitespace from A1 (common in this dataset)
    a1 = a1.rstrip(';, ').strip()

    # Strip trailing semicolons/newlines first (common in list-format data)
    a1 = a1.strip().rstrip(';\n\r').strip()

    # Protect institution names: 'Embassy of Moldova', 'Consulate of Armenia'
    # Country name is part of institution name — don't extract as geo tag
    _INST_BODY_RE = re.compile(
        r'\b(Embassy|Consulate(?:-General)?|High Commission|Mission|'
        r'Legation|Delegation|Representative Office|Trade Mission)'
        r'\s+of\s+', re.I)
    if _INST_BODY_RE.search(a1):
        _tail_country = re.search(
            r'^[^,]+\s+of\s+[A-Z][A-Za-z\s]+$', a1.strip())
        if _tail_country:
            return a1, ex_city, ex_postal, ex_country, '', ''

    # Strip place-type prefix: CITY./VILL./VILLAGE/SETTLEMENT./N.P./Rural settlement/etc.
    _PLACE_TYPE_RE = re.compile(
        r'^(?:RURAL\s+SETTLEMENT|SETTLEMENT|VILLAGE|CITY|VILL?|TOWN|STN|VLG|PGT|DP|'
        r'SELO|DEREVNYA|AULL?|KISHLAK|AYMAK|N\.P\.|K\s*/\s*COUNCIL)[.\s]+\s*', re.I)
    _a1_stripped = _PLACE_TYPE_RE.sub('', a1).strip()
    if _a1_stripped and _a1_stripped != a1:
        a1 = _a1_stripped
    # Strip trailing CITY / TOWN suffix: 'TASHKENT CITY' → 'TASHKENT', 'BAKSAN CITY' → 'BAKSAN'
    _a1_stripped2 = re.sub(r'\s+CITY\s*$', '', a1, flags=re.I).strip()
    if _a1_stripped2 and _a1_stripped2 != a1 and ',' not in a1:
        # Only strip suffix if it's the FIRST word-group (before any comma)
        pass  # handled below per-part
    ...
    def is_city_part(part):
        ...
        if ROOM_RE.search(p): return False  # Floor/Level/Unit/Suite numbers not cities
        ...
        # Exclude known state/prefecture/region names (e.g. Xinjiang, Yili)
        if p.lower() in KNOWN_STATES and p.lower() not in CITY_TO_COUNTRY: return False
        words=p.split()
    ...
    def has_geo(part):
        ...
        # Also check SSR/ASSR historical country names
        _hn = COUNTRY_NORMALIZE.get(part.strip().lower(), '')
        _hist_cou = bool(_hn and COUNTRY_TO_CODE.get(_hn.lower(), ''))
        return (cou_at_end or _hist_cou or
    ...
    def strip_tail(s, c='', p='', co='', a2=''):
        ...
        if re.fullmatch(COUNTRY_RE.pattern, s_stripped, re.I):
            ...
            return '', c, p, co, a2
        # Also handle SSR/ASSR/historical country names via COUNTRY_NORMALIZE
        _s_norm = COUNTRY_NORMALIZE.get(s_stripped.lower(), '')
        if _s_norm and COUNTRY_TO_CODE.get(_s_norm.lower(), ''):
            if not co: co = _s_norm
            return '', c, p, co, a2
        ...
        if mc_p:
            ...
            if iso_m and iso_m.group(1) in ISO_TO_COUNTRY and iso_m.group(1) not in ('No','D'):
                ...
                if mc:
                    # Skip if preceded by conjunction (street name, not geo)
                    _bmc = s[:mc.start()].rstrip()
                    _cj = re.compile(r'\b(y|e|i|u|and|ou|und|och|en|og)$', re.I)
                    if not _cj.search(_bmc):
            ...
            if m_pd and not re.match(r'^\d',s):
                # Don't extract if number follows a P.O. Box keyword
                _bmpd = s[:m_pd.start()].rstrip()
                _is_box_pd = bool(PO_BOX_RE.search(_bmpd)) or \
                             bool(re.search(r'\b(?:Box|Bag)\s*$', _bmpd, re.I))
                if not _is_box_pd:
                ...
                if uk_m and s.replace(' ','').upper().endswith(uk_str):
                    ...
                    if mp:
                        _bmp = s[:mp.start()].rstrip()
                        _is_box = bool(PO_BOX_RE.search(_bmp)) or bool(ROOM_RE.search(_bmp)) or bool(re.search(r'\b(?:Box|Bag)\s*$', _bmp, re.I))
                        if not _is_box:
                            if not p: p=re.sub(r'\.0+$','',mp.group(1))
                            s=s[:mp.start()].strip()
        # Handle 'REPUBLIC OF <word>' or '<word(s)> REPUBLIC' at end as country indicator
        if s and not co:
            ...
            if not co:
                # Handle '<adjective(s)> REPUBLIC' at end: 'KABARDINO-BALKARIAN REPUBLIC'
                _r2_m = re.search(r'\s+((?:[A-Z][\w\-]+\s+)+)REPUBLIC\b', s, re.I)
                if _r2_m:
                    _r2_cand = _r2_m.group(1).strip().title() + ' Republic'
                    _r2_norm = COUNTRY_NORMALIZE.get(_r2_cand.lower(), '')
                    if not _r2_norm:
                        # Try as 'Republic of X': e.g. 'Kabardino-Balkarian Republic' → Russia
                        _r2_norm = COUNTRY_NORMALIZE.get('republic of ' + _r2_m.group(1).strip().lower(), '')
                    if _r2_norm and COUNTRY_TO_CODE.get(_r2_norm.lower(), ''):
                        co = _r2_norm
                        s = s[:_r2_m.start()].strip()
        ...
        if s and not c:
            ...
            if mc2 and not co:
                ...
                # AND not preceded by a conjunction (e.g. 'y Colombia' = street name)
                _bmc2 = s[:mc2.start()].rstrip()
                _cj2 = re.compile(r'\b(y|e|i|u|and|ou|und|och|en|og)$', re.I)
                co2_cand=mc2.group(1)
                if (co or re.fullmatch(COUNTRY_RE.pattern, co2_cand, re.I)) and not _cj2.search(_bmc2):
        ...
        if not p and not c:
            ...
            if _spc_m:
                ...
                # Don't extract if the street part IS a P.O. Box indicator
                _st_is_box = bool(PO_BOX_RE.search(_st))
                if (_has_kw or _ends_num) and _city_known and not _st_is_box:
                    s = _st; p = _ps; c = _ct
            ...
            if (re.match(r'^[A-ZÀ-ɏ]',w) and not re.match(r'^\d',w)
                ...
                for n in [4,3,2]:
                    if len(words)>=n:
                        ...
                        if cand in MULTIWORD_CITIES and bool(co or p):
                ...
                if not c:
                    ...
                    # If previous word ends with '.' (abbrev like 'ST.'), include it in city
                    # But NOT if it's a street keyword like 'St.' (Street) or 'Rd.' (Road)
                    _prev_w = rest_of_s.split()[-1] if rest_of_s else ''
                    if (_prev_w and _prev_w.endswith('.') and len(_prev_w) <= 4
                            and not ADDR_RE.search(_prev_w.rstrip('.'))):
                        last = _prev_w + ' ' + last
                        rest_of_s = ' '.join(rest_of_s.split()[:-1])
                        already_present = last.lower() in rest_of_s.lower()
                    if (re.match(r'^[A-ZÀ-ɏ]',last) and not re.match(r'^\d',last)
                            ...
                            and (last.lower() in CITY_TO_COUNTRY or bool(co))
                            # Skip if it's only a state/province (not also a known city)
                            and not (last.lower() in KNOWN_STATES
                                     and last.lower() not in CITY_TO_COUNTRY)):
        ...
        if not p and s:
            ...
            if mp2:
                _bmp2 = s[:mp2.start()].rstrip()
                _is_box2 = bool(PO_BOX_RE.search(_bmp2)) or bool(ROOM_RE.search(_bmp2)) or \
                           bool(re.search(r'\b(?:Box|Bag)\s*$', _bmp2, re.I))
                if not _is_box2:
            ...
            for _n in range(min(4, len(_sw)), 0, -1):
                ...
                for _i in range(len(_sw)-_n+1):
                    ...
                    if ((_cl in MULTIWORD_CITIES or _cl in CITY_TO_COUNTRY)
                            ...
                            and bool(co or p)):  # only extract from n-gram scan when geo context exists
                        c = _cand
    ...
    for i,part in enumerate(parts):
        ...
        m1=re.match(r'^(\d{4,6})\s+([A-ZÀ-ɏ]\S.+)$',part)
        if m1 and not ADDR_RE.search(m1.group(2)):
            if not PO_BOX_RE.search(parts[i-1] if i>0 else ''):
        ...
        if is_city_part(part) and not city:
            ...
            nxt_state=(i+1<len(parts) and
                       (STATE_KW.search(parts[i+1]) or parts[i+1].lower() in KNOWN_STATES)
            ...
            # Also fire if it's a known city and is the only non-state/country part
            known_city = part.lower() in CITY_TO_COUNTRY
            # For multi-word parts, prv_addr only fires if city is known
            _prv_addr_ok = prv_addr and (len(part.split()) == 1 or known_city)
            if nxt or prv or nxt_state or _prv_addr_ok or known_city: city=part; continue
        ...
        _has_p_raw = (bool(re.search(r'\s\d{4,7}\.?\d*\s*$',part))
                      or (bool(_m_pd := re.search(r'\s\d{4,7}\.?\d*\s+([A-Z][a-z]+)\s*$',part))
                          and not ADDR_RE.search(_m_pd.group(1))))
        # Only trigger postal extraction if part has an address keyword
        # (prevents '1860' in 'Piazza Comitato 1860' being wrongly treated as postal)
        has_p=(_has_p_raw and (bool(ADDR_RE.search(part)) or re.match(r'^\d',part)))
        ...
        # Also catch multi-word cities: 'Jalan Ampang Kuala Lumpur' → city='Kuala Lumpur'
        words=part.split()
        if len(words)>1 and not city:
            # Multi-word city check first (2-3 word suffix)
            # Only fire when there is geo context (country/ISO found) or city is well-known
            _mw_matched = False
            for _n in [3, 2]:
                if len(words) > _n:
                    _cand = ' '.join(words[-_n:]).lower()
                    if _cand in CITY_TO_COUNTRY:
                        _rest = ' '.join(words[:-_n])
                        # Require geo context OR city is in CITY_TO_COUNTRY
                        # But NOT if preceded by a conjunction ('y Colombia', 'and France')
                        _part_has_geo = has_geo(part)
                        _city_in_ctc  = _cand in CITY_TO_COUNTRY
                        _conj_before  = bool(re.search(
                            r'\b(y|e|i|u|and|ou|und|och|en|og)\s*$',
                            _rest, re.I))
                        if ADDR_RE.search(_rest) and (_part_has_geo or _city_in_ctc) and not _conj_before:
                            city = ' '.join(words[-_n:])
                            street_parts.append(_rest)
                            _mw_matched = True
                            break
            if _mw_matched: continue
        if len(words)>1 and not city:
            last=words[-1]
            # Handle "District/City" slash pattern (e.g. "Tuzla/Istanbul")
            if '/' in last and last.count('/') == 1:
                sp = last.split('/')
                # Both parts must be real words (>1 char) — not B/N, N/A, etc.
                _sp_valid = (len(sp[0]) > 1 and len(sp[1]) > 1
                             and not re.match(r'^[BbNn]/[BbNn]$', last)  # B/N pattern
                             and not re.match(r'^[A-Z]/[A-Z]$', last))   # any X/Y single chars
                if (_sp_valid and re.match(r'^[A-ZÀ-ɏ]', sp[0]) and
            ...
            if (re.match(r'^[A-ZÀ-ɏ]',last) and not re.match(r'^\d',last)
                    ...
                    and (last.lower() in CITY_TO_COUNTRY or has_geo(part))
                    and part.lower() not in CITY_TO_COUNTRY):  # don't split known city names
                rest=' '.join(words[:-1])
    ...
    if not street_parts and not city and not postal and not country and len(parts)==1:
        # Check if the whole A1 is a known city name
        _a1_low = a1.strip().lower()
        if _a1_low in CITY_TO_COUNTRY and not ADDR_RE.search(a1):
            city = a1.strip()
            country = normalize_country(CITY_TO_COUNTRY[_a1_low])
        else:
    ...
    # Post-loop: if entire A1 resolved to a single known city with no other content,
    # promote it from street_parts to city
    if not city and len(street_parts)==1 and not postal and not country:
        _sp = street_parts[0].strip()
        if _sp.lower() in CITY_TO_COUNTRY and is_city_part(_sp):
            city = _sp
            street_parts = []
    ...
    # Strip trailing CITY/TOWN suffix from extracted city value
    if city: city = re.sub(r'\s+(?:CITY|TOWN)\s*$', '', city, flags=re.I).strip()
...
def infer_country(city, state, postal):
    ...
    if re.search(r'\b(Oblast|Krai|Okrug|Bashkortostan|Tatarstan|Dagestan|'
        ...
        return 'Russia'
    # Also infer Russia for well-known Russian REGION names
    _RU_REGIONS = {
        'altai','amur','arkhangelsk','astrakhan','belgorod','bryansk','chelyabinsk',
        'chita','chukotka','irkutsk','ivanovo','kaluga','kamchatka','kemerovo','kemerovsk',
        'khabarovsk','kirov','kostroma','krasnoyarsk','kurgan','kursk',
        'lipetsk','magadan','moscow','murmansk','nizhny novgorod','novgorod',
        'novosibirsk','omsk','orenburg','oryol','penza','perm','primorsky','pskov',
        'rostov','ryazan','sakhalin','samara','saratov','smolensk','stavropol',
        'tambov','tomsk','tula','tver','tyumen','ulyanovsk','vladimir','volgograd',
        'vologda','voronezh','yaroslavl','zabaykalsky','altai region','krasnodar',
        'crimea','autonomous republic of crimea','chuvash republic','mordovia',
        'kabardino-balkarian','kabardino-balkar','north ossetia','ingushetia',
        'jewish autonomous','kalmyk','adygea','karachay-cherkess',
    }
    # Ukrainian regions (check before Russian to avoid Kherson → Russia)
    _UA_REGIONS = {
        'kyiv region','kharkov region','kharkiv region','dnipropetrovsk region',
        'donetsk region','zaporizhie region','zaporizhzhia region','lviv region',
        'odessa region','sumy region','poltava region','cherkassy region',
        'zhytomyr region','vinnitsa region','ternopol region','kherson region',
        'mykolaiv region','khmelnytsky region','chernigov region','lugansk region',
        'luhansk region','ivano-frankov region','kirovograd region',
    }
    for _ur in _UA_REGIONS:
        if _ur in s.lower(): return 'Ukraine'
    for _rr in _RU_REGIONS:
        if _rr in s.lower(): return 'Russia'
    ...
    # Korean provinces
    _KR_PROVS = {
        'gangwon province','gangwon-do','gyeonggi province','gyeonggi-do',
        'chungcheongnam-do','chungcheongbuk-do','jeollanam-do','jeollabuk-do',
        'gyeongsangnam-do','gyeongsangbuk-do','jeju province',
        'north chungcheong','south chungcheong','north gyeongsang','south gyeongsang',
        'north jeolla','south jeolla',
    }
    if s.lower() in _KR_PROVS or c.lower() in _KR_PROVS:
        return 'South Korea'
...
_STATE_LEVEL = {
    'province':1,'region':1,'oblast':1,'krai':1,'kray':1,
    'governorate':1,'prefecture':1,'okrug':1,'wilayat':1,'emirate':1,
    'department':1,'canton':1,'voivodeship':1,'republic':1,'shire':1,
    'administrative':1,'zone':1,'area':1,'sector':1,'circuit':1,
    'cercle':1,'subregion':1,'constituency':1,
    'county':2,'district':2,'borough':2,'township':2,'ward':2,
    'rayon':2,'raion':2,'gun':2,'parish':2,'commune':2,'division':2,
    'arrondissement':2,'tehsil':2,'taluk':2,'subdistrict':2,'muhafazah':2,
    'woreda':2,'kebele':2,'sub-city':2,'subcity':2,'subcounty':2,
    'upazila':2,'thana':2,'mandal':2,'zila':2,
    'ku':2,'shi':1,'cho':2,'machi':2,  # Japanese: ku=ward, shi=city, cho=town
    'maakond':1,'laan':1,'landkreis':1,'lan':1,'fylke':1,'amt':1,  # Nordic/Baltic county
    'vald':2,'küla':2,'dong':2,'ri':2,'myeon':2,'gu':2,  # Estonian parish/village, Korean district
}
_STATE_LEVEL_RE = re.compile(
    r'\b(' + '|'.join(_STATE_LEVEL.keys()) + r')\b', re.I)

def _get_state_level(p):
    m = _STATE_LEVEL_RE.search(p)
    return _STATE_LEVEL.get(m.group().lower(), 99) if m else 99

def split_state_hierarchy(st):
    """Split 'County X, Province Y' → (state='Province Y', addr3='County X').
    Only fires when there is at least one level-1 (broad) AND one level-2 (specific) part."""
    if ',' not in st:
        return st, ''
    parts = [p.strip() for p in st.split(',')]
    levels = [(_get_state_level(p), p) for p in parts]
    has_l1 = any(l == 1 for l, _ in levels)
    has_l2 = any(l == 2 for l, _ in levels)
    if not (has_l1 and has_l2):
        return st, ''
    broad    = [p for l, p in levels if l == 1]
    specific = [p for l, p in levels if l == 2]
    unknown  = [p for l, p in levels if l == 99]
    return ', '.join(broad + unknown), ', '.join(specific)

def extraction_confidence(value, field_type, country=''):
    """Return HIGH / MEDIUM / LOW confidence for an extracted value.
    Only HIGH confidence extractions are acted upon in the pipeline.
    """
    if not value: return 'LOW'
    v = str(value).strip()
    if field_type == 'postal':
        if not v: return 'LOW'
        if JUNK_POSTAL.match(v): return 'LOW'
        if len(v) < 4: return 'LOW'
        if re.match(r'^\d{4,7}$', v): return 'HIGH'
        if UK_POST_RE.match(v): return 'HIGH'
        if re.match(r'^[A-Z]{1,2}\d', v): return 'MEDIUM'
        return 'LOW'
    if field_type == 'city':
        if v.lower() in CITY_TO_COUNTRY: return 'HIGH'
        if len(v) < 3 or re.match(r'^\d', v): return 'LOW'
        if ADDR_RE.search(v) or STATE_KW.search(v): return 'LOW'
        if len(v.split()) > 3: return 'LOW'   # too long to be just a city
        return 'MEDIUM'
    if field_type == 'state':
        if v.lower() in KNOWN_STATES: return 'HIGH'
        if bool(STATE_KW.search(v)) and len(v.split()) <= 4: return 'HIGH'
        return 'MEDIUM'
    if field_type == 'country':
        if v.lower() in COUNTRY_TO_CODE: return 'HIGH'
        if bool(COUNTRY_RE.search(v)): return 'HIGH'
        return 'LOW'
    return 'LOW'
    ...
    return dict(
        ...
        KNOWN_STATES=KNOWN_STATES,
        CITY_TO_COUNTRY=CITY_TO_COUNTRY,
        MULTIWORD_CITIES=MULTIWORD_CITIES,
    )
...
with st.sidebar:
    ...
    if uploaded:
        try:
            ...
            # Strip float suffix (.0) from all cells — pandas converts int columns with NaN to float
            import re as _re
            df_raw = df_raw.apply(lambda col: col.map(
                lambda v: _re.sub(r'\.0$', '', str(v)) if _re.match(r'^\d+\.0$', str(v)) else v))
...
def run_osm_validation(df, col_map):
    ...
    for idx_row in all_rows:
        ...
        for col_key, label, allowed_types in FIELD_CONFIG:
            ...
            if not col:
                continue
            val = str(df.at[idx_row, col]).strip()
            if not val or val.lower() in ('nan', ''):
                continue

            log = {
                'entity_id':     str(row.get(C('entity_id'), '') or ''),
                'row':           idx_row + 1,
                'field':         label,
                'place':         val,
                'country_hint':  country_hint,
                'osm_type':      '',
                'osm_display':   '',
                'expected_types': '/'.join(sorted(allowed_types)) if allowed_types else 'any',
                'status':        'OK',
                'note':          '',
            }
            ...
            if log['status'] in ('MISMATCH', 'NOT_FOUND', 'CORRECTED'):
                ...
                if existing and existing not in ('nan', ''):
                    df.at[idx_row, 'EXCEPTION_FLAG'] = existing + ' | ' + flag_val
                else:
                    df.at[idx_row, 'EXCEPTION_FLAG'] = flag_val

    return OSM_VALIDATION_LOG


COMPLETENESS_LOG = []   # one entry per missing / data-loss case

def run_completeness_check(df_raw, df_clean, col_map):
    """
    Compare every row in the cleaned dataframe against its original raw row.
    For each address field: if the raw row had a non-blank value and the
    cleaned row is blank, flag it as DATA_LOSS.
    Also flags rows where ADDRESS1 is blank but raw had content across
    any address column.

    Never modifies any field values.
    Results written to COMPLETENESS_LOG and EXCEPTION_FLAG.
    """
    COMPLETENESS_LOG.clear()
    C = lambda f: col_map.get(f, '')

    # Address fields to check — in order of importance
    ADDR_FIELDS = [
        ('address',    'ADDRESS1'),
        ('address2',   'ADDRESS2'),
        ('address3',   'ADDRESS3'),
    ...
    # Build a lookup: (entity_id, address_id) → raw row
    id_col  = C('entity_id')
    aid_col = C('address_id')

    def row_key(row):
        eid = str(row.get(id_col, '') or '').strip()
    ...
    for idx_row, clean_row in df_clean.iterrows():
        k = row_key(clean_row)
        raw_row = raw_index.get(k)
        if raw_row is None:
            # New row (multi-address split) — skip raw comparison
            continue

        for col_key, label in ADDR_FIELDS:
            col = C(col_key)
            if not col:
                continue

            raw_val   = str(raw_row.get(col, '')   or '').strip().replace('nan', '')
            clean_val = str(clean_row.get(col, '') or '').strip().replace('nan', '')

            # Skip if raw was already blank — nothing to lose
            if not raw_val:
                continue

            # DATA_LOSS: raw had a value, clean is now blank
            if not clean_val:
                log_entry = {
                    'entity_id': str(clean_row.get(id_col, '') or ''),
                    'address_id': str(clean_row.get(aid_col, '') or ''),
                    'row':        idx_row + 1,
                    'field':      label,
                    'raw_value':  raw_val,
                    'clean_value': '',
                    'issue':      'DATA_LOSS',
                    'note':       f"Raw had {raw_val!r} but cleaned value is blank",
                }
                COMPLETENESS_LOG.append(log_entry)

                # Flag on row
                flag_val = f"DATA_LOSS:{label}"
                existing = str(df_clean.at[idx_row, 'EXCEPTION_FLAG']).strip() if 'EXCEPTION_FLAG' in df_clean.columns else ''
                if existing and existing not in ('nan', ''):
                    df_clean.at[idx_row, 'EXCEPTION_FLAG'] = existing + ' | ' + flag_val
                else:
                    df_clean.at[idx_row, 'EXCEPTION_FLAG'] = flag_val

            # DATA_CHANGED: cleaned value is different from raw
            # (log for audit but don't flag — changes are expected from cleaning)
            elif clean_val.lower() != raw_val.lower():
                log_entry = {
                    'entity_id':  str(clean_row.get(id_col, '') or ''),
                    'address_id': str(clean_row.get(aid_col, '') or ''),
                    'row':        idx_row + 1,
                    ...
                    'issue':      'CHANGED',
                    'note':       'Value was modified during cleaning',
                }
                COMPLETENESS_LOG.append(log_entry)
...
def validate_pass2(df, col_map, diffs, CITY_TO_COUNTRY, COUNTRY_TO_CODE,
                   ...
                   JUNK_POSTAL, UK_POST_RE, use_geo_verify=False):
            ...
            if new_a3_2 and new_st2 != st:
                ...
                if pc_state2 and pc_state2 != st:
                    ...
                    if not cty and pc_city2 and C('city'):
                        # Verify with geo lookup if city not in static list
                        _pc_city_l = pc_city2.lower()
                        ...
                        if _place_type == 'city' and _place_conf in ('high','medium'):
                            ...
                            cty = pc_city2
                            # Do NOT update session-level CITY_TO_COUNTRY with OSM results
                            # as this would affect classification of subsequent rows
                            ...
                            # Do NOT update session-level KNOWN_STATES with OSM results
...
if run:
    ...
    def get(row, col):
        v = str(row.get(C(col),'') or '').strip()
        # Strip float suffix added by pandas when reading integer columns with NaN
        if v.endswith('.0') and re.match(r'^\d+\.0$', v):
            v = v[:-2]
        return v
    ...
    def setcol(idx, field_key, val):
        if C(field_key):
            # Log if clearing a field that had a value (audit trail)
            if val == '' and C(field_key) in df.columns:
                old = str(df.at[idx, C(field_key)] or '').strip()
                if old and old not in ('nan','NaT',''):
                    diffs.append({'Row':idx+1,'Field':C(field_key),'Before':old,'After':'(cleared)'})
            df.at[idx, C(field_key)] = val
    ...
    for idx, row in df.iterrows():
        ...
        # ── 0b. Strip trailing semicolons from ADDRESS1 ─────────────
        if a1 and a1.rstrip().endswith(';'):
            clean_semi = a1.rstrip().rstrip(';').strip()
            setcol(idx,'address', clean_semi); a1 = clean_semi

        # ── 1. Normalize COUNTRY ──────────────────────────────────
        # Per user rule: if COUNTRY is already populated, keep the raw value as-is.
        # Only normalize when COUNTRY is blank (will be filled by later steps).
        pass  # step 1 disabled — raw country value preserved
        ...
        if st_ and _L2_KW.search(st_) and not _L1_KW.search(st_):
            # Pure level-2 unit in STATE → find best available address field
            _2b_val   = st_
            _2b_moved = False
            for _2b_f in ['address3', 'address2', 'address']:
                _2b_col = C(_2b_f)
                if _2b_col:
                    _2b_ex = str(df.at[idx, _2b_col]).strip()
                    if not _2b_ex or _2b_ex in ('nan',''):
                        record(idx, _2b_f, '', _2b_val)
                        setcol(idx, _2b_f, _2b_val)
                        _2b_moved = True
                        break
            # Always clear STATE for level-2 units, even if no address field free
            setcol(idx,'state',''); st_=''
        ...
        # ── 7. Fix ADDRESS2 ───────────────────────────────────────
        # Clear A2 if it duplicates A1 (happens when split fires but A2 gets the whole address)
        ...
        if a2:
            ...
            if STATE_KW.search(a2) and not ADDR_RE.search(a2) and not st_:
                ...
                if extraction_confidence(a2,'postal') in ('HIGH','MEDIUM'):
                    ...
                    df.at[idx,'EXCEPTION_FLAG'] = 'AMBIGUOUS_POSTAL_IN_A2'
            elif (not cty and not ADDR_RE.search(a2) and not COUNTRY_RE.search(a2)
                    and not re.match(r'^\d',a2) and len(a2.split(','))<=2):
                record(idx,'city','',a2)
                setcol(idx,'city', CITY_NORMALIZE.get(a2.lower(), a2)); cty=a2
                setcol(idx,'address2',''); a2=''

        # ── 7b. Move STATE → CITY if STATE clearly contains a city name ─
        st_now = str(df.at[idx,C('state')] if C('state') else '').strip()
        if st_now and C('state') and C('city'):
            _cty_now = str(df.at[idx,C('city')] if C('city') else '').strip()
            _CITY_SUFFIX = re.compile(r'\bCity\s*$', re.I)
            _ends_city  = bool(_CITY_SUFFIX.search(st_now))
            _in_ctc     = st_now.lower() in CITY_TO_COUNTRY
            _in_known_st= st_now.lower() in KNOWN_STATES
            _has_st_kw  = bool(STATE_KW.search(st_now))
            # Move if: ends ' City' with no STATE_KW, OR known city but not a known state
            _should_move = ((_ends_city and not _has_st_kw) or
                            (_in_ctc and not _in_known_st and not _has_st_kw))
            if _should_move:
                if not _cty_now:  # CITY is blank → move STATE there
                    record(idx,'city','',st_now)
                    setcol(idx,'city', CITY_NORMALIZE.get(st_now.lower(), st_now)); cty=st_now
                    setcol(idx,'state','');    st_=''
                elif _cty_now.lower() != st_now.lower():  # CITY already has a value
                    # STATE is a city-type value — move to ADDRESS3 if it's sub-level
                    # (e.g. 'Dalseo District' already in CITY, 'Daegu City' in STATE)
                    pass  # leave it — pipeline will handle via step 8b
        ...
        # ── 8b. Split STATE hierarchy: 'County X, Province Y' → STATE + ADDRESS3 ──
        st_now = str(df.at[idx,C('state')] if C('state') else '').strip()
        if st_now and ',' in st_now:
            _new_st, _new_a3 = split_state_hierarchy(st_now)
            if _new_a3 and _new_st != st_now:
                record(idx,'state',st_now,_new_st)
                setcol(idx,'state',_new_st); st_=_new_st
                if C('address3'):
                    _ex_a3 = str(df.at[idx,C('address3')] if C('address3') else '').strip()
                    _merged = ((_ex_a3 + ', ' + _new_a3) if _ex_a3 else _new_a3).strip(', ')
                    setcol(idx,'address3',_merged)
            else:
                # No L2/L1 hierarchy split — try parse_city_field:
                # handles 'Nikki, Borgou Department' → city='Nikki', state='Borgou Department'
                # Only fire if at least one comma-part has NO state keyword (it's a city)
                _parts_8b = [p.strip() for p in st_now.split(',') if p.strip()]
                _has_non_kw = any(not STATE_KW.search(p) for p in _parts_8b)
                _cty_now = str(df.at[idx,C('city')] if C('city') else '').strip()
                ...
                if _has_non_kw and _pc_state and _pc_state != st_now:
                    ...
                    if not _cty_now:
                        ...
                        if _pc_city and C('city'):
                            ...
                            setcol(idx,'city', CITY_NORMALIZE.get(_pc_city.lower(), _pc_city)); cty=_pc_city
                        if _pc_a3 and C('address3'):
                            _ex_a3 = str(df.at[idx,C('address3')] if C('address3') else '').strip()
        ...
        if a1 and not NOTE_RE.match(a1):
            ...
            # Only restore A1 if CITY is already set AND city was the ONLY thing extracted
            # (no country/postal). If other fields were extracted too, let A1 be cleaned.
            _city_blocked = bool(new_city and cty and new_city.lower() != cty.lower())
            _only_city_extracted = _city_blocked and not new_country and not new_postal
            if clean_a1 != a1 and _only_city_extracted:
                pass  # only a blocked city was stripped — keep original A1 intact
            elif clean_a1 != a1:
                record(idx,'address',a1,clean_a1)
                setcol(idx,'address',clean_a1)

            if new_city and not cty:
                if extraction_confidence(new_city,'city') in ('HIGH','MEDIUM'):
                    record(idx,'city','',new_city)
                    setcol(idx,'city', CITY_NORMALIZE.get(new_city.lower(), new_city)); cty=new_city
                else:
            ...
            if new_postal and not pst:
                if extraction_confidence(new_postal,'postal') in ('HIGH','MEDIUM'):
        ...
        # ── 11b. A1 is a known state/province name → move to STATE ──────
        a1_now = str(df.at[idx,C('address')] if C('address') else '').strip()
        if a1_now and C('state') and not ADDR_RE.search(a1_now):
            # Case 1: A1 is a single KNOWN_STATES value
            if a1_now.lower() in KNOWN_STATES and a1_now.lower() not in CITY_TO_COUNTRY:
                _st_now = str(df.at[idx,C('state')] if C('state') else '').strip()
                if not _st_now:
                    record(idx,'state','',a1_now)
                    setcol(idx,'state',a1_now); st_=a1_now
                    setcol(idx,'address',''); a1=''
            # Case 2: A1 is all comma-parts that are KNOWN_STATES (e.g. 'Yili, Xinjiang')
            elif ',' in a1_now:
                _11b_parts = [p.strip() for p in a1_now.split(',') if p.strip()]
                _all_ks = all(p.lower() in KNOWN_STATES and p.lower() not in CITY_TO_COUNTRY
                               for p in _11b_parts)
                if _all_ks:
                    _st_now = str(df.at[idx,C('state')] if C('state') else '').strip()
                    # Put the last (most general) part in STATE, earlier parts in ADDRESS3
                    _ks_state = _11b_parts[-1]
                    _ks_rest  = ', '.join(_11b_parts[:-1])
                    if not _st_now:
                        record(idx,'state','',_ks_state)
                        setcol(idx,'state',_ks_state); st_=_ks_state
                    if _ks_rest and C('address3'):
                        _ex_a3 = str(df.at[idx,C('address3')] if C('address3') else '').strip()
                        if not _ex_a3:
                            setcol(idx,'address3',_ks_rest)
                    setcol(idx,'address',''); a1=''
        ...
        # ── 12c. Validate CITY — reclassify if it's actually a STATE or postal ─
        _cty_12c = str(df.at[idx,C('city')] if C('city') else '').strip()
        if _cty_12c and C('city'):
            _st_12c  = str(df.at[idx,C('state')] if C('state') else '').strip()
            _pst_12c = str(df.at[idx,C('postal')] if C('postal') else '').strip()
            _in_ctc  = _cty_12c.lower() in CITY_TO_COUNTRY
            _is_kst  = _cty_12c.lower() in KNOWN_STATES
            ...
            # If city is actually a postal code
                ...
                if not _pst_12c:
                    record(idx,'postal','',_cty_12c)
                    setcol(idx,'postal',_cty_12c)
                    setcol(idx,'city','')
                    df.at[idx,'EXCEPTION_FLAG'] = 'CITY_WAS_POSTAL'
        ...
        if C('country_id'):
            if fi and not ISO2_RE.match(fi):
                ...
                if not code:
                    # Try normalizing first (handles 'IRAN, ISLAMIC REPUBLIC OF' etc.)
                    _fc_norm = normalize_country(fc)
                    code = get_country_code(_fc_norm)
                if code: setcol(idx,'country_id',code)

        # ── 15. Postal validation + reclassify if invalid ──────────
        if o_postal and C('postal'):
            _pst_val = str(df.at[idx,C('postal')]).strip()
            _cou_val = str(df.at[idx,C('country')] if C('country') else '').strip()
            _pflag   = validate_postal(_pst_val, _cou_val)
            df.at[idx,'POSTAL_FLAG'] = _pflag
            # Step 3 spec: if postal is invalid, attempt reclassification
            if _pflag == 'INVALID' and _pst_val:
                # Is it actually a city name?
                if _pst_val.lower() in CITY_TO_COUNTRY and not str(df.at[idx,C('city')] if C('city') else '').strip():
                    record(idx,'city','',_pst_val)
                    setcol(idx,'city',_pst_val)
                    setcol(idx,'postal','')
                    df.at[idx,'POSTAL_FLAG'] = 'MOVED_TO_CITY'
        ...
        # Ensure EXCEPTION_FLAG exists for this row
        if 'EXCEPTION_FLAG' not in df.columns:
            df['EXCEPTION_FLAG'] = ''
    ...
    # Ensures data is never stranded in ADDRESS2 with ADDRESS1 empty.
    prog.progress(0.90, text="Compacting address fields…")
    _ca1 = C('address');   _ca2 = C('address2');  _ca3 = C('address3')
    # If ADDRESS1 blank and CITY contains what looks like address content
    # (not in city lookup), move CITY → ADDRESS1
    if _ca1:
        for _cidx in df.index:
            _cv1 = str(df.at[_cidx, _ca1]).strip() if _ca1 else ''
            _ccty = str(df.at[_cidx, C('city')]).strip() if C('city') else ''
            if _cv1 in ('', 'nan') and _ccty and C('city'):
                # CITY has content, A1 is blank — check if it's really address content
                _cty_is_addr = bool(ADDR_RE.search(_ccty))
                _cty_in_ctc  = _ccty.lower() in CITY_TO_COUNTRY
                if _cty_is_addr and not _cty_in_ctc:
                    df.at[_cidx, _ca1] = _ccty
                    df.at[_cidx, C('city')] = ''
                    diffs.append({'Row':_cidx+1,'Field':'ADDRESS1','Before':'','After':_ccty,'Pass':'compact'})

    if _ca1 and (_ca2 or _ca3):
        for _cidx in df.index:
            _v1 = str(df.at[_cidx, _ca1]).strip() if _ca1 else ''
            _v2 = str(df.at[_cidx, _ca2]).strip() if _ca2 else ''
            _v3 = str(df.at[_cidx, _ca3]).strip() if _ca3 else ''
            if _v1 in ('', 'nan') and _v2 not in ('', 'nan'):
                # Shift: A2 → A1, A3 → A2
                df.at[_cidx, _ca1] = _v2
                diffs.append({'Row':_cidx+1,'Field':'ADDRESS1','Before':'','After':_v2,'Pass':'compact'})
                if _ca2:
                    df.at[_cidx, _ca2] = _v3 if _v3 not in ('', 'nan') else ''
                    if _v3 not in ('', 'nan'):
                        diffs.append({'Row':_cidx+1,'Field':'ADDRESS2','Before':_v2,'After':_v3,'Pass':'compact'})
                    else:
                        diffs.append({'Row':_cidx+1,'Field':'ADDRESS2','Before':_v2,'After':'','Pass':'compact'})
                if _ca3:
                    df.at[_cidx, _ca3] = ''
            elif _v1 in ('', 'nan') and _v2 in ('', 'nan') and _v3 not in ('', 'nan'):
                # Only A3 has content → shift to A1
                df.at[_cidx, _ca1] = _v3
                diffs.append({'Row':_cidx+1,'Field':'ADDRESS1','Before':'','After':_v3,'Pass':'compact'})
                if _ca3:
                    df.at[_cidx, _ca3] = ''
                    diffs.append({'Row':_cidx+1,'Field':'ADDRESS3','Before':_v3,'After':'','Pass':'compact'})
    ...
    run_completeness_check(df_raw, df, col_map)
    _loss_count    = sum(1 for r in COMPLETENESS_LOG if r['issue'] == 'DATA_LOSS')
    _changed_count = sum(1 for r in COMPLETENESS_LOG if r['issue'] == 'CHANGED')
    if _loss_count:
        st.warning(
            f"⚠️ Completeness check: **{_loss_count} data loss** "
            f"(field had value in raw but is blank after cleaning) — "
            f"see **Completeness Check** sheet in download"
        )
    else:
        st.success(
            f"✅ Completeness check passed — no data loss detected. "
            f"{_changed_count} values were modified during cleaning."
        )
...
if 'cleaned_df' in st.session_state:
    ...
    with tab2:
        if diffs:
            st.dataframe(pd.DataFrame(diffs), width='stretch', height=400)
        else:
    ...
    with tab3:
        ...
        # Exceptions summary
        if 'EXCEPTION_FLAG' in df.columns:
            exc = df[df['EXCEPTION_FLAG'].str.strip() != ''][['ENTITY_ID','ADDRESS_ID','EXCEPTION_FLAG']]
            if not exc.empty:
                st.caption(f"⚠️ {len(exc)} rows flagged for review")
                st.dataframe(exc, width='stretch', height=200)
        buf = io.BytesIO()
        with pd.ExcelWriter(buf, engine='openpyxl') as w:
            ...
            if OSM_VALIDATION_LOG:
                _osm_cols = ['entity_id','row','field','place','country_hint',
                             'osm_type','expected_types','status','note','osm_display']
                osm_df = pd.DataFrame(OSM_VALIDATION_LOG)
                for _c in _osm_cols:
                    if _c not in osm_df.columns: osm_df[_c] = ''
                osm_df = osm_df[_osm_cols]
                osm_df.columns = [
                    'ENTITY_ID','Row','Field','Value','Country Hint',
                    'OSM Type','Expected Types','Status','Note','OSM Display Name'
                ]
                osm_df.to_excel(w, sheet_name='OSM Validation', index=False)
            if COMPLETENESS_LOG:
                _comp_cols = ['entity_id','address_id','row','field',
                              'raw_value','clean_value','issue','note']
                comp_df = pd.DataFrame(COMPLETENESS_LOG)
                for _c in _comp_cols:
                    if _c not in comp_df.columns: comp_df[_c] = ''
                comp_df = comp_df[_comp_cols]
                comp_df.columns = [
                    'ENTITY_ID','ADDRESS_ID','Row','Field',
                    'Raw Value','Cleaned Value','Issue','Note'
                ]
                comp_df.to_excel(w, sheet_name='Completeness Check', index=False)
        st.download_button(
...
KNOWN_STATES = {'mn', 'tasmania', 'county tyrone', 'southampton', 'nizhegorodskaya oblast', 'yunnan province', 'paktia', 'jiangxi', 'tehsil', 'istanbul province', 'xinjiang province', 'mordovia', 'occupied palestinian territories', 'herat province', 'baja california', 'karnataka', 'bursa', 'cirebon', 'kachin state', 'the autonomous island of anjouan union of comoros', 'hatay', 'puerto rico', 'nidwalden', 'selkirkshire', 'eastern mindanao', 'chekyabinsk oblast', 'barcelona', 'wenchuan county', 'palm jum eirah', 'gilan', 'xinjiang uyghur autonomous region', 'lvivska oblast', 'guangdong province', 'hà nam', 'lesnoy', 'ulyanovsk region', 'colorado', 'alicante', 'western bahr el ghazal', 'tver oblast', 'smolensk', 'kish island', 'kolkata', 'kandahar i̇li', 'kamchatski krai', 'merseyside', 'altay republic', 'leicestershire', 'tulskaya obl.', 'guannan county', 'worcester', 'xuanen county', 'zabol province', 'west yorkshire', 'hounslow', 'la convención', 'sverdlov oblast', 'southern shan state', 'al asimah governorate', 'ciudad real', 'saratov oblast', 'irkutskaya oblast', 'dodoma region', 'u.s. virgin islands', 'ayeyarwaddy division', 'chuquisaca', 'vakaga', 'gauteng', 'sirri island', 'burnham on crouch', 'jalal-abad region', 'burntisland', 'ba ria-vung tau province', 'samsun', 'bryanskaya oblast', 'republic of karachayevo-cherkessian federation', 'huelva', 'khyber paktunkhwa province', 'istanbul', 'chon buri', 'region of hama', 'zhambyl region', 'diyarbakir', 'selangor state', 'mean cheay', 'malang', 'co armagh', 'oxfordshire', 'liaoning sheng', 'sutherland', 'united kingdom', 'leningrad oblast', 'farah', 'tarlac province', 'gaza', 'samarskaya, oblast', 'champasak province', 'donetsk', 'yakutia', 'donetsk people’s republic', 'paktika eyaleti', 'perugia', 'inner mongolia', 'vologda oblast', 'tehsil bölgesi', 'kuala lumpur', 'saga', 'nordrhein-westfalen', 'makarashtra', 'yangonsichuan', 'east riding of yorkshire', 'al ahmadi governorate', 'clayton', 'southern darfur', 'primorski region', 'orlovskaya oblast', 'wilmington', 'khyber paktunkhwa', 'mugello', 'lorestan', 'grodnenskaya oblast', 'kray primorskiy', 'seoul', 'gangwon-do', 'moskovskaya zastava municipal okrug', "south p'yo'ngan province", 'hunan', 'zaporizhzhia region', 'krasnodar', 'redbridge', 'yueyang county', 'brest oblast', 'i̇dlib valiliği', 'kentucky', 'victoria', 'chuvash republic', 'mahe island', 'kasai province', 'shetland islands', 'orenburg oblast', 'kanagawa', 'estado de mexico', 'jawa tengah', 'kursk region', 'oryol oblast', 'jateng', 'minas gerais', 'east kazakhstan region', 'provincia de catamarca', 'selangor', 'northern mariana islands', 'mersin', 'western mindanao', 'denbighshire', 'cumbria', 'sussex', 'worcestershire', 'port said', 'baijiao county', 'kamchatskiy krai', 'helwan', 'kizilorda', 'fct', 'zurich', 'sacatepequez', 'county antrim', 'santa catarina', 'surry', 'herat eyaleti', 'hulhumale', 'high peak', 'kabil i̇li', 'west yorks', 'rostov-on-don oblast', 'hubei sheng', 'prov. kabul', 'provinsi nangarhar', 'pongchon', 'hertford', 'hants', 'abkhazia', 'alaska', 'tambov region', 'anhui', 'baokang county', 'giza', 'nimruz province', 'nghe an province', 'shropshire', 'yunnan', 'essex', 'liangxi district', 'south yorkshire', 'vologodskaya oblast', 'tamaulipas', 'republic of udmurtia', 'sakhalinskaya oblast', 'zabaykalskiy kray', 'sumatera utara', 'nusa kambangan island', 'new mexico', 'stafford', 'murmansk region', 'tengah', 'naypyidaw union territory', 'doğu londra', 'penang', 'republic of karachayevo', 'cross river state', 'xx', 'ar krim', 'holguín', 'miaoli county', 'punjab province', 'new brunswick', 'south kivu province', 'madrid', 'mississippi', 'west midlands', 'northwest frontier province', 'kemerovo oblast', 'dong nai province', 'rif dimashq', 'metro manila', 'south glamorgan', 'qom province', 'karachi', 'yangon', 'gansu', 'alborz province', 'maine', 'lothian', 'south sinai', 'jiangzi province', 'east london', 'shabwah', 'tehran', 'ohio', 'sagaing', 'tanintharyi region', 'ajetlate island', 'luhansk region', 'luanda', 'kaduna state', 'kursk oblast', 'kaluga oblast', 'junín', 'assam', 'dagestan', 'zhlobin gomel region', 'district of columbia', 'jinxian county', 'artvin', 'ningxia', 'solnechnogorsk region', 'north yorkshire', 'anthohomadinika atsimo', 'jalisco', 'west  midlands', 'ilfov', 'jaipur', 'maharashtra', 'northern ireland', 'fang', 'kandal', 'ningxia hui autonomous region', 'sistan and baluchistan province', 'rajasthan', 'federal capital territory', 'bioko norte', 'republic of chechnya', 'hainan', '34460', 'haryana', 'grodno oblast', 'xinjiang uyghur autonomous region (xuar)', 'san luis potosi', 'persekutuan', 'cavite', 'zelenograd', 'carmarthen', 'fata', 'kolomna region', 'sundon', 'fujian', 'permskiy krai', 'basilan province', 'hayling island', 'sukhumi district', 'nabul', 'novosibirsk region', 'west equatoria state', 'zhenjiang', 'changbai korean autonomous county', 'kelantan', 'banten', 'taiz governorate', 'new south wales', 'ajeltake islands', "north hamgyo'ng province", 'altai krai', 'county londonderry', 'lot et garonne', 'ryazan oblast', 'isle of wight', 'county clare', 'tain', 'rakhine state', 'st andrew', 'cairo', 'konya', 'samara samarskaya oblast', 'adygea republic', 'yorkshire', 'rakhine', 'north sumatra', 'strathclyde', 'minsk region', 'gallen canton', 'sakha republic (yakutia)', 'kyrgyz republic', 'illinios', 'tula region', 'sonora', 'abian', 'bali', 'altai territory', 'amur', 'western province of rwanda', 'hongkong', 'chengdu', 'xinxiang', 'zabaykalskiy krai', 'kyoto', 'provinsi hadramawt', 'badakhshan province', 'selatan', 'labuan', 'nizhniy novgorod region', 'guizhou province', 'shanghai', 'maryland', 'hebei sheng', 'zeytinburnu district', 'clwyd', 'new providence', 'new territory', 'khuzestan', 'greate r londo', 'khabarovsk krai', 'jonglei state', 'montserrado county', 'saint petersburg region', 'north west', 'north kivu province', 'minnesota', 'norwich', 'majuro ajeltake island', 'gansu province', 'iloilo province', 'ryazan region', 'north dakota', 'kaliningrad oblast', 'tx', 'krasnoyarsk krai', 'altayskiy krai', 'khatlon province', 'transnistria', 'guangning county', 'tokyo', 'arbil', 'north east', 'the autonomous republic of crimea', 'geneva', 'lagos', 'leningradskaya obl.', 'birmingham', 'co', 'df. distrito federal', 'province panama', 'région de nizhny novgorod', 'federally administered tribal areas (fata)', 'valais', 'dawley', 'arkhangelsk region', 'baluchistan i̇li', 'sofia oblast', 'berkshire', 'rahdhebai magu', 'primorskiy kray', 'hertfordshire', 'altaiskii krai', 'avenue', 'mon state', 'vaud', 'shandong', 'moskva', 'south hamgyong province', 'rostov region', 'perm territory', 'county armagh', 'aleppo', 'buzau', 'west java', 'durham', '江西省', 'western australia', 'novosibirskaya oblast', 'middx', 'batı mindanao', 'koch unity state', 'northamptonshire', 'st. gallen', 'al hudaydah governorate', 'co durham', 'udmurtia republic', 'naifaru', 'gibraltar', 'perm krai', 'bushehr province', 'primorski krai', 'arkansas', 'shangdong', 'myanmar', 'provincia de alajuela', 'haeudong', 'yaroslavl region', 'samara', 'rainham', 'sverdlovsk', 'murmasnkaya oblast', 'sverlovskaya oblast', 'jizzakh region', 'federal capital territory (fct)', 'lavan island', 'khartoum', 'yakutiya', 'republic of maris', 'west virginia', 'nangarhar province', 'udmurtskaya resp', 'severnaya osetiya-alaniya', 'kwazulu natal', 'sergiev posad district, moscow oblast', 'voronezh region', 'midlothian', 'channel islands', 'santa cruz', 'delaware', 'western cape', 'karachayevo-cherkessia', 'saanxi', 'west australia', 'orenburgskaya oblast', 'vicenza', 'feltham', 'region: gaza', 'negros oriental', 'carabobo state', 'lviv region', 'harrow', 'down', 'province sindh', 'kutahya', 'peterborough', 'wp labuan', "donetsk people's republic", 'dorset', 'republic kabardino-balkaria', 'vitosha region', 'biskra', 'tyumenskaya oblast', 'são paulo', 'florida', 'gyeonggi-do', 'uusimaa', 'san josé', 'ulyanovsk oblast', 'karatay', 'maharasthra', 'republic of dagestan', 'inner mongolia autonomous region', 'altai kray', 'mariy el republic', 'leicester', 'hadramut eyaleti', 'quezon i̇li', 'khakasia republic', 'xinjiang uygur autonomous region', 'sejong', 'south australia', 'vargas', 'chih', 'north gaza governorate', 'yelabuga region', 'izmir', 'jiangxi province', 'wirral', 'khan younis region', 'lancashire', 'province gombe', 'dadra and nagar haveli and daman and diu', 'batken oblast', 'koui ouham-pendé prefecture', 'arbat municipal okrug', 'ebonyi state', 'vitebsk region', 'helmand i̇li', 'ilam province', 'republic of tatarstan', 'hadhramout', 'surrey', 'beirut', 'tianjin', 'xinjiang uighur autonomous region', 'south kalimantan', 'nisantasi', 'northern territory', 'tyne and wear', 'kowoloon', 'chesterfield', 'herat', 'mwali', 'greater manchester', 'east susse', 'wadi khaled', 'bushehr', 'abia state', 'telangana', 'west midland', 'republic of sakha', 'mazandaran', 'carmarthenshire', 'damascus countryside', 'negeri sembilan', 'armagh', 'bay', 'kaliningrad region', 'krasnodarskiy kray', 'khouzestan', 'sistan and balochistan', 'astrakhan region', 'nueva ecija province', 'republic of bashkortostan', 'qasim', 'damascus province', 'vitebsk region/oblast', 'lanarkshire', 'anhui sheng', 'lipetsk', 'vostochno-kazakhstanskaya oblast', 'cornwall', 'nueva ecija', 'nimruz i̇li', 'kafia kingi', 'leningrad region', 'region: west bank', 'nusa tenggara barat', 'vermont', 'berlin', 'distrito capital', 'georgia', 'chihuahua', '九龍', 'nottinghamshire', 'magadan oblast', 'bamingui-bangoran', 'sulu', 'lobaye province', 'voronezh oblast', 'bago', 'federal administered tribal areas', 'darfur', 'nuevo leon', 'central island', 'quanjiao county', 'helmand', 'sistan and baluchestan', 'hong kong', 'hong kong special administrative region', 'saskatchewan', 'republic of karachayevo-cherkessia', 'parana', 'taiwan', 'van can commune', 'sheerness', 'co londonderry', 'liaoning', 'guangxi province', 'shetland', 'puducherry', 'finedon', 'ica', 'tarlac', 'mari el republic', 'tartous', 'kirkuk province', 'island east', 'marib governorate', 'wilmslow', 'hadramawt', 'f.c.t.', 'hebei', 'chungcheongnam-do', 'nishni novgarod region', 'schwyz', 'suez', 'khanty-mansi autonomous okrug yugra', 'castle bromwich', 'krasnoyarski kr', 'bizkaia', 'oldham', 'java central', 'altay krai', 'mount lebanon', 'lower juba region', 'kabardino-balkarian republic', 'shabwah governorate', 'ahuachapán', 'sosnovyy bor', 'amazonas', 'suffolk', 'xinjiang uighur autonomous regionn', 'an trai village', 'kazan oblast', 'gilan province', 'county down', 'minworth', 'tibet autonomous region', 'hormozgan', 'barming', 'saratov region', 'doma', 'thuan province', 'stanway', 'région de yangon', 'preah', 'arkhangelsk oblast', 'hubei', 'golestan', 'utah', 'kabylie region', 'vitebskaya oblast', 'moscow region', 'washington', 'jilin province', 'johor', 'sao paulo', 'nimroz', 'dyfed', 'gujarat', 'brussels', 'dunbartonshire', 'kabul', 'semyonovskii admiralteiskii region', 'timur', 'republic of karelia', 'rudgwick', 'cleveland', 'minsk region/oblast', 'sakhalinsk oblast', 'antrim county', 'derby', 'heilongjiang province', 'voromezh oblast', 'barnacle', 'hong kong island', 'kostroma oblast', 'kabul province', 'hovedstaden', 'karaçay çerkes cumhuriyeti', 'kaliningradskaya oblast', 'anbar bölgesi', '11185', 'isfahan county', 'north waziristan', 'cheshire', 'caithness', 'gyeongsangnam-do', 'quebec', 'sentosa', 'saint petersburg', 'cayo coco', 'damascus-countryside', 'chafford hundred', 'flintshire', 'laguna', 'jiangsu', 'pakhtunkhwa province', 'sulu region', 'luhansk oblast', 'northwestern', 'moscow oblast', 'shehong county', 'amurskaya oblast', 'nizhni novgorod oblast', 'ivanovo oblast', 'deir ez-zor governorate', 'monza and brianza', 'washington state', 'manitoba', 'tinh province', 'shanliurfa', 'clackmannanshire', 'tatarstan resp', 'hebei province', 'zhukovsky region', 'yaroslavsk oblast', 'brest reg', 'gulin county', 'rasaught', 'mitcham', 'crimea', 'jilin', 'tatarstan republic', 'lipetsk oblast', 'norton', 'stavropol krai', 'ross-shire', 'krasnodarski krai', 'yamalo-nenets autonomous okrug', 'tenerife', 'tower hamlets', 'karaganda oblast', 'south humberside', 'buryatia', 'ayeyarwaddy', 'sichuan sheng', 'metro manilla', 'tortola', 'republic of crimea', 'kabiliye bölgesi', 'kabardino-balkar republic', 'kherson oblast', 'yugan county', 'harju county', 'kandahar province', 'balochistan province', 'tula oblast', 'vakaga prefecture', '1111020', 'khanh hoa', 'north osetia-alania republic', 'new addington', 'anbar province', 'tambov oblast', 'alexandria', 'tennessee', 'odisha', 'northwest federal region', 'andhra pradesh', 'central kabul', 'ongar', 'i̇stanbul', 'khanty-mansisyk autonomous okrug-yugra', 'county durham', 'novgorodskaya oblast', 'aden', 'republic of mordovia', 'beijing', 'gwent', 'bergamo', 'shunyi district', 'west bengal', 'gloucester', 'kamchatka krai', '34467', 'henan province', 'gaza strip', 'huntington', 'northern gaza', "al-bayda' governorate", 'matanzas', 'hampshire', 'glasgow', 'quillacollo', 'oblast novosibirskaya', 'gloucestershire', 'mumbai', 'staffordshire', 'syrdarya region', 'trung', 'samara oblast', 'zaporizhia region', 'miami', 'kingstown', 'jakarta', 'ndjamena', 'turkmen jebel region', 'kildare', 'unity state', 'hutton', 'yaroslavl oblast', 'virginia', 'catamarca', 'south austrailia', 'primorskiy krai', 'maluku', 'haslemere', 'zulia', 'north osetia-alania', 'moheli', 'primorskiy region', 'san francisco', 'sumatera barat', 'tver', 'south holland', 'jiangsu sheng', 'diyali', 'oxford', 'kabylie', 'homs', 'kent', 'vladimir region', 'sicuan', 'sumut', 'al-mahrah governorate', 'hillmorton', 'khimki region', 'wilayah persekutuan kuala lumpur', 'cochabamba', 'south east london', 'menouba governorate', 'liujiang county', 'ncd', 'junin', 'lagos state', 'herts', 'new hampshire', 'north carolina', 'kerala', 'karachay-cherkess republic', 'shaanxi province', 'quezon', 'dagestan rep.', 'rhondda cynon taf', 'esfahan', 'pskov oblast', 'conwy county borough', 'kayah', 'hawali governorate', 'qidong, jiangsu province', 'lantian county', 'xuyi county', 'hamgyong province', 'novgorod region', 'nizhny novgorod', 'udmurtskaya', 'central', 'chukotka autonomous oblast', 'california', 'hama governorate', 'kayin state', 'shandong province', 'al anbar province', 'rostov oblast', 'the central', 'karachayevo-cherkessia republic', 'handforth', 'dudley', 'grodno region', 'kocaeli', 'wales', 'xiangyin country', 'guangzhou city guangdong province', 'trujillo', 'republic of sakha (yakutia)', 'smolensk oblast', 'newham', 'thimarafushi', 'fars province', 'zaporizhzhia oblast', "chandler's ford", 'villimale', 'shanxi', 'st martin', 'fl', 'east azerbaijan', 'qinghe', 'kurdistan', 'new delhi', 'guangxi zhuangzuzizhiqu', 'ontario', 'anbar', 'federal territory of labuan', 'guerrero', 'haut-uolo', 'nicosia', 'provincia: luanda', 'cundinamarca', 'nimroz province', 'krasnodar krai', 'nana-grebizi', 'hertfordshire,', 'helmand province', 'west jakarta', 'samut prakan', 'federally administered tribal area', 'yashnobod region', 'solnechnogorskiy region', 'shaanxi', 'dong nai', 'mordovia republic', 'canouan island', 'qidong', 'province of havana', 'khyber pakhtunkhwa', 'nimruz', 'south hwanghae province', 'zhaosu county', 'beni territory', 'ny', 'bueng kum', 'mallorca', 'sindh province', 'archangelskaya oblast', 'canberra act', 'qandahar province', 'central java', 'bryansk region', 'anbar region', 'syrian arab republic', 'khojir region', 'ninh thuan province', 'woodham', 'al maryah island', '1202', 'central administrative okrug', 'namangan', 'kirovskaya', 'rotherham', 'republic of udmurtskaya', 'khanty-mansi autonomous okrug - yugra', 'indiana', 'chelyabinsk region', 'british virgin islands', 'kabardino-balkaria republic', 'sverdlovsk region', 'volgogradskaya oblast', 'bas-uolo', 'louisiana', 'utrecht', 'caba', 'sichuan province', 'angus', 'chuvash republic-chuvashia', 'nizhny novgorod oblast', 'london', 'missouri', 'ombella-mpoko province', 'provincia y departamento de lima', 'north kivu', 'smolensk region', 'uygur autonomous region', 'karelia republic', 'solihull', 'antalya', 'azad jammu and kashmir', 'surbiton', 'alsace', 'queensland', 'ireland', 'cross river', 'ramsey', 'tenggara', 'guangdong', 'karen state', 'anjouan', 'tokyo-to', 'kharg island', 'brestskaya', 'zaporizhskiy region', 'south ossetia', 'mandalay region', 'alberta', 'heilongjiang', 'denton', 'tibet', 'zabaikalsk krai', 'battersea', 'chungcheongbuk-do', 'guangdonng', 'idaho', "lugansk people's republic", 'moskovskaya obl.', 'chonburi province', 'castor', 'gwynedd', 'krasnodar region', 'north of lebanon', 'west lothian', 'turkish republic of northern cyprus', 'west java province', '34956', 'demerara', 'kangwon province', 'sverdlovskaya obl.', 'longgang', 'korolev oblast', 'east sussex', 'quintana roo', 'yangon region', 'pursat province', 'southsea', 'stanmore', 'sverdlovskaya oblast', 'buslingthorpe', 'provinsi dki jakarta', 'chita region', 'new territories', 'yaroslav oblast', 'perth', 'belgorod oblast', 'mengjin county', 'maranhao', 'omsk region', 'delhi', 'villa clara', 'texas', 'north darfur', 'provinsi bali', 'county of st. george east', 'hadramout province', 'nizhniy novgorod oblast', 'nangarhar', 'chechnya', 'massachusetts', 'misnk region', 'provinsi jawa tengah', 'kiashahr gilan province', 'jersey', 'guangxi', 'alpujarras', 'novgorod oblast', 'baja', 'australian capital territory', 'kemerovo region', 'qinghai province', 'republica de panama', 'kunar', 'crowle', 'paramaribo', 'md', 'nizhegorodskaya', 'sichuan', 'salai county', 'gaziantep', 'chuy region', 'mogilev oblast', 'bolton', "p'yo'ngan province", 'warwickshire', 'port-au-prince', 'tartus', 'naypyidaw', 'hong kong sar', 'zabaykalsky krai', 'west glamorgan', 'archangelsk region', 'kherson region', 'somerset', 'chaguanas', 'hawaii', 'buryatiya', 'pyongyang', 'd.1 nizhny novgorod region', 'new waltham', 'south kivu', 'saratovskaya oblast', 'attica', 'republic of vanuatu', 'zabul', 'wilayah persekutuan labuan', 'new york', 'kansas', 'tamps', 'renfrewshire', 'maranhão', 'feixi county', 'kirov region', 'rutshuru territory', "al bayda' governorate", 'saada governorate', 'donetsk oblast', 'gomelskaya oblast', 'coahuila', 'torfaen', 'aberdeenshire', 'i̇zmir', 'olancho', 'karelia', 'voronezhskaya oblast', 'republic of north ossetia-alania', 'amran', 'bago region', 'north rhine-westphalia', 'ulam dong', 'khanty-mansiysky avtonomnyi', 'grand cayman', 'mandalay province', 'michigan', 'xingshan county', 'am dafock', 'gaoqing county', 'vale of glamorgan', 'nizhny novgorod region', 'weston-super-mare', 'co. donegal', 'harworth', 'nova scotia', 'tehsil rinala khurd', 'sakha republic', 'ankara', 'kirovskaya obl.', 'tver region', 'stirlingshire', 'kalmykia republic', 'novgorod', 'roundhay', 'ingushetia', 'serdika region', 'bukharskaya oblast', 'novosibirsk oblast', 'kandahar', 'penzenskaya oblast', 'bashkortostan republic', 'shangrao jiangxi province', 'lima', 'stirling', 'shenzhen', 'ayrshire', 'sulu bölgesi', 'fergana region', 'county fermanagh', 'volga federal region', 'scottish borders', 'new malden', 'kirov oblast', 'nebraska', 'south wales', 'guandong', 'hadramawt governorate', 'kowloon', 'manchester', 'primorsky kray', 'sverdlovskaya obl', 'shan state', 'kermanshah', 'baluchistan province', 'yamalo-nenets autonomous area', 'stockport', 'chukotka', 'enugu', 'ajeltake', 'vladimir oblast', 'khanty-mansi autonomous region', 'moscow', 'chechen republic', 'gaza governorate', 'kangan', 'rangoon', 'amsterdam', 'republic of udmurt', 'chukotka autonomous okrug', 'iowa', 'ouest', 'republic of mari el', 'keserwan-jbeil', 'damascus', 'madhya pradesh', 'amur oblast', 'sumatera utar', 'firouzkouh county', 'pakistan border region', 'baluchistan', 'irkutsk', 'salalah dhofar governorate', 'jalisc', 'glamorgan', 'arab republic', 'nanjing county', 'middlesex', 'lanao del sur', 'northwich', 'harju maakond', 'al-anbar', 'west azerbaijan', 'hhohho', 'bryansk oblast', 'ayeyarwady', 'osmaniye', 'republic of chuvashia', 'wyoming', 'mahe', 'bedfordhsire', 'al-anbar province', 'primorsky krai', 'hama region', 'parana state', 'sind', 'north humberside', 'hoa binh', 'kedah', 'devon', 'ivanov oblast', 'almhahra', 'guangzhou', 'sandy', 'samara region', 'bristish columbia', 'south dakota', 'british-colombia', 'antigua', 'new jersey', 'sweida', 'baleares', 'guizhou', 'pennsylvania', 'suqatra', 'gazze şeridi', 'hama', 'donetsk peoples republic', 'avon', 'wicklow', 'takal region', 'lincoln', 'aichi-ken', 'il', 'mount lebanon governorate', 'admiralteiskii region', 'sucre', 'south governorate', 'ismailia', 'binlang county', 'kampala', 'ecija province', 'cayos de villa clara', 'federally administered tribal areas', 'kalmykia', 'shanxi province', 'laian county', 'west sussex', 'billingham', 'al anbar', 'kwazulu-natal', 'ut', 'pahang', 'xinjiang', 'yili', 'lancs', 'leningradskaya oblast', 'hong kong (sar)', 'sagaing region', 'anhua', 'tehran province', 'moskovskaya', 'north ossetia-alania republic', 'gyengsangbuk-do', 'provinsi jawa timur', 'wiltshire', 'tatarstan', 'londonderry county', 'coleraine county', 'preah sihanouk', 'lidingö', 'sosnovski region', 'kurgan region', 'mid glamorgan', 'quezon province', 'zhejiang sheng', 'castel', 'santiago', 'irkutsk region', 'yanling county', 'islamabad', 'north hwanghae province', 'middlesbrough', 'vernon', 'ha tinh province', 'navoiy region', 'paktika province', 'dahyan governorate', 'isle of man', 'santa ana', 'tyumen region', 'gholestan', 'leeds', 'east java', 'jammu and kashmir', 'goa', 'magway', 'american samoa', 'sulawesi tengah', 'greater london', 'udmurtia', 'gomelskaya', 'udmert republic', 'mangistauskaya oblast', 'island of mohéli', 'gomel region', 'vitebsk oblast', 'muscat governorate', 'st. nicholas at wade', 'pristina', 'lida oblast', 'lahore', 'khaborovsk krai', 'bromley', 'baidoa bay region', 'kolpino', 'kohgiluyeh and boyer-ahmad', 'balochistan', 'ha nam province', 'raa atoll', 'northumberland', 'katanga', 'kalyazinski region', 'komi republic', 'brest region', 'kohkiluye-va-boyer', 'derbyshire', 'badong county', 'in', 'lincolnshire', 'kalhaidhoo', 'tomsk oblast', 'sutton coldfield', 'guatemala', 'chelyabinsk oblast', 'wilayah persekutuan', 'fryazino region', 'penza oblast', 'tomilino', 'sverdlovsk oblast', 'gao region', 'krasnodar territory', 'bangui', 'almaty region', 'thornton heath', 'uttar pradesh', 'guangdong sheng', 'ks', 'kostanayskaya oblast', 'minsk oblast', 'huailai county', 'milan', 'zhejian', 'norfolk', 'hemlington', 'lusaka province', 'county cavan', 'gia lai province', 'krasnoyarsk region', 'bashkortostan', 'mandalay', 'kohgaloye and boyer ahmad province', 'henan', 'kemerovo oblast-kuzbass', 'anhui province', 'arizona', 'teesside', 'primorsky region', 'radstock', 'sindh', 'yorskhire', 'zabaikalski kr.', 'mahé', 'oxon', 'leyte', "ta'izz governorate", 'weihei', 'isle of skye', 'samarskaya oblast', 'gombe', 'alabama', 'herefordshire', 'kaluga region', 'sampsonievskoe', 'buckingham', 'irkutsk oblast', 'neamț county', 'astrakhan oblast', 'autonomous republic of crimea', 'sakarya province', 'isfahan', 'suchitepequez', 'sakhalin oblast', 'kurgan oblast', 'ramenskoye region', 'moskovskaya oblast', 'guangxi zhuang autonomous region', 'departamento de san salvador', 'udmurt republic', 'oklahoma', 'sharqia', 'nevada', 'ajeltake island', 'perthshire', 'lampung', 'chuvashia', 'jiayu county', 'jawa timur', 'autonomes island of anjouan', 'fars', 'hunan province', 'pangasinan province', 'baghdad', 'mahrashtra', 'buckinghamshire', 'british columbia', 'menaka gao region', 'idlib province', 'fujian province', 'zuid-holland', 'kayin', 'crimea republic', 'kordestan', 'ha nam', 'jiangsu province', 'skane lan', 'delta state', 'isfahan province', 'connecticut', 'himachal pradesh', 'antrim', 'state of santa catarina', 'murmansk', 'tamil nadu', 'volgograd oblast', 'kensington & chelsea', 'tyne & wear', 'limete', 'northern darfur', 'tyrone', 'baja california sur', 'tyumen oblast', 'odintsovo district', 'quartier socimat', 'kabardino-balkaria', 'west bank', 'kuala lumpur wilayah persekutuan', 'st. george', 'sidi bel abbes', 'jewish autonomous oblast', 'nam định', 'barmal paktika province', 'khuzestan province', 'liaoning province', 'naypyitaw', 'thurgau', 'semnan province', 'cremona', 'deir ez-zor valiliği', 'nan an', 'perm region', 'vasilevskiy island', 'rostovskaya oblast', 'north hamgyong province', 'ancash', 'perak', 'abyan governorate', 'leigh-on-sea', 'murmansk oblast', 'severo-kazakhstanskaya oblast', 'co down', 'manila', 'qinghai', 'manchester greater', 'pichincha', 'state of selangor', 'north chungcheong', 'nangarhar eyaleti', 'idlib governorate', 'bedfordshire', 'kunar province', 'east yorkshire', 'yuzhno-sakhalinsk oblast', 'mandalay division', 'hadramawt province', 'hwanghae province', 'ben arous', 'zhejiang', 'gyunggi', 'baghdad governorate', 'rhode island', 'sergipe', 'sinaloa', 'sakarya', 'warrington', 'omsk oblast', 'gomel oblast', 'veracruz', 'oregon', 'south carolina', 'tekirdag', 'palestinian territories', 'lattakia governorate', 'veziristan', 'wisconsin', 'rizal', 'almaty oblast', 'nayarit', 'thanh hoa province', 'salaj', 'northern khorasan province', 'hubei province', 'moskovskaya obl', 'xixiang', 'republic of buryatia', 'zhejiang province', 'vienna', 'feydhoo', 'karaj alborz province', 'northants', 'chongren county', 'punjab', 'michoacan', 'castilla-la mancha', 'alborz', 'vasilyevski island', 'south lanarkshire', 'andhra pardesh', 'mari el', 'central america', 'greenford', 'province guangdong', 'durango', 'stavropolskiy krai', 'enugu state', 'banten province', 'bay region', 'huanjiang county', 'davao', 'kingswood', 'paktia province', 'nethern antilles', 'federally administered tribal areas(fata)', 'cambridgeshire', 'pulau pinang', 'montana', 'winnersh', 'shantou guangdong province', 'incheon', 'chattisgarh', 'hainan province', 'shahin shahr', 'tula', 'jawa barat', 'nanshan zhiyuan', 'illinois', 'republic of bashkiria'}
